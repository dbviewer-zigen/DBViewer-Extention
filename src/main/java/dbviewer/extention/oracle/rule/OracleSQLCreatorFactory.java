...
public class OracleSQLCreatorFactory extends DefaultSQLCreatorFactory {
	public OracleSQLCreatorFactory() {
		super();
	}
...
//		sb.append(SQLUtil.encodeQuotation(column.getName()));
	...
	protected String getCreateTableStr() {
		...
		if(table instanceof Synonym){
			Synonym syn = (Synonym)table;
			StringBuffer sb = new StringBuffer();
			sb.append("CREATE SYNONYM ");
			sb.append(getTableNameWithSchemaForSQL(table, isVisibleSchemaName));
			sb.append(" FOR");
			sb.append(syn.getSynonymInfo().getTable_owner());
			sb.append(".");
			sb.append(syn.getSynonymInfo().getTable_name());
			setDemiliter(sb);
			return sb.toString();
		}else{
			return super.getCreateTableStr();
		}
	}
