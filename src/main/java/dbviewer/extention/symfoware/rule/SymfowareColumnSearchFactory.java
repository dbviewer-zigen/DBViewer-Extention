...
public class SymfowareColumnSearchFactory extends DefaultColumnSearcherFactory {
	public SymfowareColumnSearchFactory() {
		super();
	}
	...
	protected String getCustomColumnInfoSQL(String dbName, String owner, String table) {
		...
		sb.append("        ,COL.NULLABLE ").append(NULLABLE_STR);;
		sb.append("    FROM");
		...
		// add
		sb.append("        OR T.DB_NAME = 'RDBII_DICTIONARY')");
		sb.append("        AND T.SCHEMA_NAME = '" + SQLUtil.encodeQuotation(owner) + "'");
		sb.append("        AND T.TABLE_NAME = '" + SQLUtil.encodeQuotation(table) + "'");

		sb.append("        AND T.DB_CODE = COL.DB_CODE");
		sb.append("        AND T.SCHEMA_CODE = COL.SCHEMA_CODE");
	...
	public TableColumn[] execute(Connection con, String schemaPattern, String tableName) throws Exception {
		...
		try {
			...
			String sql = getCustomColumnInfoSQL(dbName, schemaPattern, tableName);
			...
			while (rs.next()) {
				...
				column.setColumnName(rs.getString(COLUMN_NAME_STR));
				column.setDataType(getJavaType(rs.getString(DATA_TYPE_STR)));
				column.setTypeName(getTypeName(rs.getString(TYPE_NAME_STR)));
				column.setColumnSize(rs.getInt(DATA_PRECISION_STR));
				column.setDecimalDigits(rs.getInt(DATA_SCALE_STR));
				column.setDefaultValue(rs.getString(DATA_DEFAULT_STR));
				String remarks = rs.getString(COMMENTS_STR);
				if (convertUnicode) {
					remarks = JDBCUnicodeConvertor.convert(remarks);
				}
				column.setRemarks(remarks);

				if ("NO".equals(rs.getString(NULLABLE_STR).trim())) {
					column.setNotNull(true);
				} else {
					column.setNotNull(false);
				}
